...
public sealed class AppServerStartLifecycleTests
{
    [Fact]
    public async Task CreateInitializedAsync_WhenInitializeFails_DisposesProcessAndRpc()
    {
        ...
        var act = async () => await CodexAppServerClient.CreateInitializedAsync(
            options,
            process,
            rpc,
            NullLogger.Instance,
        ...
        await act.Should().ThrowAsync<CodexAppServerInitializeException>();
