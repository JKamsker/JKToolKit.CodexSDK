...
- [x] T011 Implement request/response transformer pipeline in app-server core.
- [x] T012 Implement notification transformer pipeline + pluggable notification mappers + raw notification stream.
- [x] T013 Add `CallAsync<T>` + raw notification helpers on `CodexAppServerClient`/`CodexTurnHandle`.
- [x] T014 Add unit tests covering override ordering + safety.
...
- [x] T021 Audit other high-traffic endpoints (threads/turns/skills/apps/config/mcp mgmt) for param/shape mismatches and harden parsers.

## Phase 3 — Generator tool (schema → internal wire DTOs)

- [x] T030 Add `JKToolKit.CodexSDK.UpstreamGen` tool project + CLI skeleton.
- [x] T031 Implement schema discovery + metadata output (Codex version + schema hash).
- [x] T032 Generate internal C# wire DTOs from `codex_app_server_protocol.schemas.json` and commit output under `src/JKToolKit.CodexSDK/Generated/Upstream/`.
- [x] T033 Refactor app-server internals to use generated DTOs where it reduces drift risk (while keeping public wrappers stable).
- [x] T034 Add generator `--check` mode + CI guard (fails if generated output is stale).
- [x] T035 Make generated schema metadata deterministic + OS-portable (no absolute paths / timestamps).

## Phase 4 — Upstream release sync automation

- [x] T040 Add `UPSTREAM_CODEX_VERSION.txt` pin + local developer docs.
- [x] T041 Add GitHub workflow: detect new `@openai/codex` version → generate schema → run generator → build/test → open PR only if green.
...
- [x] T051 Add McpServer parsing override hooks (tools list + reply parsing) + tests.
- [x] T052 Document override hooks with examples (AppServer/Exec/McpServer).
