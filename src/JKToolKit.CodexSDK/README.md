...
2. `ICodexProcessLauncher` starts the process (`codex exec ... -` for legacy stdin-prompt mode, or `codex exec ... "<prompt>"` when `PromptArgument` is used)
...
Newer Codex CLI builds support passing the prompt as an argument while still piping separate stdin content. The SDK exposes that via `PromptArgument` and `StdinPayload`:

```csharp
var options = new CodexSessionOptions("<repo-path>", prompt: null)
{
    PromptArgument = "Review this patch for correctness issues.",
    StdinPayload = File.ReadAllText("patch.diff"),
    Model = CodexModel.Gpt52Codex
};
...
Use `Prompt` when you intentionally want the legacy stdin-prompt (`codex exec -`) flow.
