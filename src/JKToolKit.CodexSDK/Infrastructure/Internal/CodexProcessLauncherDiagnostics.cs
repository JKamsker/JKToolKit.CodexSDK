...
internal static class CodexProcessLauncherDiagnostics
{
    internal static string CreateDiagnosticMessage(string? stderr, string executablePath) =>
        string.IsNullOrWhiteSpace(stderr)
            ...
            : $"Failed to start Codex CLI process using executable '{executablePath}'. Stderr (first 2000 chars, redacted): {CodexDiagnosticsSanitizer.Sanitize(stderr, maxChars: 2000)}";
}
